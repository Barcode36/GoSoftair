...
/**
     ...
     * @return boolean
     */
    public function query($query) {     //////////////////////////////  LA USIAMO MAI?????????????????
   ...
   /**
    ...
    public function getResultAssoc() {
        if ($this->_result != false) {
            $numero_righe=rowCount($this->_result);
    ...
    public function getResult() {
        if ($this->_result!=false) {
            ...
            debug('Numero risultati:'. $numero_righe);
            if ($numero_righe>0) {
                $row = fetch($this->_result);			//oppure fetchAll
                $this->_result=false;
                return $row;
            }
        }
    ...
    public function getObject() {
        ...
        debug('Numero risultati:'. $numero_righe);
        if ($numero_righe>0) {
            $row = fetch($this->_result,$this->_return_class);
            $this->_result=false;
            return $row;
        } else
            return false;
    ...
    public function getObjectArray() {
        ...
        if ($numero_righe>0) {
            $return=array();
            while ($row =fetch($this->_result,$this->_return_class)) {
                $return[]=$row;
            }
            $this->_result=false;
            return $return;
