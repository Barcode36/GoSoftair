...
class CRicerca {
	...
	/**
     * Seleziona sul database gli utenti
     * in base ai punti accumulati con le vittorie
     *
     * @return string
     */
    public function listautenti(){
        ...
        if ($risultato!=false) {
            $array_risultato=array();
            foreach ($risultato as $item) {
                $tmpUtente=$FUtente->load($item->username);
                $array_risultato[]=array_merge(get_object_vars($tmpUtente));
            }
            $view->impostaDati('datiut',$array_risultato);
        }
        $view->impostaDati('task','listautenti');
        return $view->processaTemplate();
    }
    /**
